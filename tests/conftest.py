...
def _auth_secrets_are_fake(monkeypatch):
    ...
    # 🔴 The same trap one layer further in, added 2026-08-16 with the recovery alias. This is
    # EMPTY in the repo and is NOT empty on the owner's machine. Left unpinned, the day he sets
    # `ADMIN_RECOVERY_EMAILS=carlosavillah90@gmail.com` in `.env`, the test named "a non-UACH
    # account is refused" starts passing that very address — and the suite reports the gate as
    # sealed while `.env` is the only thing holding it. A test that wants the configured case
    # sets it explicitly with its own monkeypatch.
